...
export default function DateRangeView({ dates, onSelectDate }: DateRangeViewProps) {
      ...
      {queried && stats && !loading && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <h3 className="text-2xl font-bold text-slate-900">{stats.totalProcessed.toLocaleString()}</h3>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <h3 className="text-2xl font-bold text-slate-900">{stats.annotatedCount.toLocaleString()}</h3>
            <p className="text-sm text-slate-500 mt-1">已标注</p>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="text-2xl font-bold text-slate-900">{stats.unannotatedCount.toLocaleString()}</h3>
            <p className="text-sm text-slate-500 mt-1">未标注</p>
          </div>
        </div>
      )}

      {/* Label Distribution */}
      {queried && stats && Object.keys(stats.labelDistribution || {}).length > 0 && !loading && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
            <h3 className="text-sm font-bold text-slate-800">标签分布（聚合）</h3>
          </div>
          <div className="p-6 space-y-2">
            {(Object.entries(stats.labelDistribution) as [string, number][])
              .sort(([, a], [, b]) => b - a)
              .map(([label, count]) => {
                const maxCount = Math.max(...(Object.values(stats.labelDistribution) as number[]));
                const pct = maxCount > 0 ? Math.round((count / maxCount) * 100) : 0;
                return (
                  <div key={label} className="flex items-center gap-3">
                    <span className="text-sm text-slate-700 w-24 truncate shrink-0">{label}</span>
                    ...
                    <span className="text-xs text-slate-500 w-10 text-right shrink-0">{count}</span>
                  </div>
                );
              })}
          </div>
